...
router.post('/register', async(req, res) => {
    ...
    return res
        ...
        try {
            ...
            if (phone)
            return res.status(400).json({success: false, message: 'Số điện thoại đã có người sử dụng'});
            ...
            console.log(error)
            res.status(500).json({success: false, message: `Đăng nhập thất bại${error}`})
        }
})
...
router.post('/login', async(req, res) => {
    ...
    return res
        .status(400)
        .json({ success: false, message: 'Số điện thoại hoặc password không chính xác !!!'});

        try {
            const user = await User.findOne({phoneNumber})
            if(!user)
            return res.status(400).json({success: false, message: 'Số điện thoại hoặc password không chính xác !!!'});
...
router.put('/:id', verifyToken, async(req, res) => {
    ...
    try {
        ...
        // nếu kh đúng user
        if(!updatedProfile)
            return res.status(401).json({ success: false, message: 'update thất bại' });
